...
use ratproto_lexicon::{BodyDef, FieldSchema, ParamsDef, ProcedureDef, QueryDef};
...
pub fn gen_query(ctx: &GenContext<'_>, def_name: &str, query: &QueryDef) -> Result<String, String> {
    ...
    let return_type = if has_output {
        format!("Result<{type_name}Output, ratproto_xrpc::Error>")
    } else if has_binary_output {
        "Result<Vec<u8>, ratproto_xrpc::Error>".to_string()
    } else {
        "Result<(), ratproto_xrpc::Error>".to_string()
    };

    writeln!(
        out,
        "pub async fn {fn_name}(client: &ratproto_xrpc::Client{params_arg}) -> {return_type} {{",
...
pub fn gen_procedure(
    ...
    let mut args = String::from("client: &ratproto_xrpc::Client");
    if has_input {
    ...
    if is_blob_input {
        if has_output {
            ...
            writeln!(
                out,
                "    serde_json::from_value(v).map_err(|e| ratproto_xrpc::Error::Xrpc {{ status: 0, error: \"DeserializationError\".to_string(), message: e.to_string() }})"
            )
            .ok();
        } else {
            writeln!(
...
fn gen_endpoint_object(
    ctx: &GenContext<'_>,
    type_name: &str,
    obj: &ratproto_lexicon::ObjectDef,
...
mod tests {
    ...
    fn gen_get_profile_query() {
        ...
        if let ratproto_lexicon::Def::Query(q) = &schema.defs["main"] {
    ...
    fn gen_create_record_procedure() {
        ...
        if let ratproto_lexicon::Def::Procedure(p) = &schema.defs["main"] {
