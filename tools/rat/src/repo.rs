...
use ratproto_api::com::atproto::{SyncGetRepoParams, sync_get_repo};
use ratproto_cbor::{Cid, Value};
use ratproto_repo::Commit;
use ratproto_xrpc::Client;
use serde::Serialize;
...
fn read_car(path: &str) -> Result<CarData> {
    ...
    let (roots, blocks) =
        ratproto_car::read_all(reader).with_context(|| "failed to read CAR file")?;
...
fn walk_mst_node(
    ...
    let value =
        ratproto_cbor::decode(data).with_context(|| format!("failed to decode MST node: {cid}"))?;
...
async fn export(args: ExportArgs) -> Result<()> {
    ...
    let dir = ratproto_identity::Directory::new();
    let identity = dir
        .lookup_did(&did)
        .await
