...
impl From<shrike_cbor::CborError> for StreamError {
    fn from(e: shrike_cbor::CborError) -> Self {
...
pub fn parse_firehose_frame(data: &[u8]) -> Result<Event, StreamError> {
    use shrike_cbor::Decoder;
    use shrike_syntax::{Did, Handle, Tid};
...
fn extract_frame_header(header: shrike_cbor::Value<'_>) -> Result<(i64, String), StreamError> {
    use shrike_cbor::Value;
...
fn require_int(fields: &Fields<'_>, key: &str) -> Result<i64, StreamError> {
    for (k, v) in fields {
        if *k == key {
            return match v {
                shrike_cbor::Value::Unsigned(n) => i64::try_from(*n)
                    .map_err(|_| StreamError::ParseCbor(format!("field {key:?} overflows i64"))),
                shrike_cbor::Value::Signed(n) => Ok(*n),
                _ => Err(StreamError::ParseCbor(format!(
...
fn optional_bool(fields: &Fields<'_>, key: &str) -> Option<bool> {
    for (k, v) in fields {
        if *k == key
            && let shrike_cbor::Value::Bool(b) = v
        {
            return Some(*b);
        }
    }
...
fn parse_commit_blocks(
    ...
    let (_roots, blocks) = shrike_car::read_all(&blocks_bytes[..])
...
fn extract_cid_optional(fields: &Fields<'_>, key: &str) -> Option<shrike_cbor::Cid> {
    for (k, v) in fields {
        if *k == key {
            return match v {
                shrike_cbor::Value::Cid(c) => Some(*c),
                _ => None,
            };
        }
    }
...
fn extract_bytes(fields: &Fields<'_>, key: &str) -> Option<Vec<u8>> {
    for (k, v) in fields {
        if *k == key
            && let shrike_cbor::Value::Bytes(b) = v
        {
...
mod tests {
    ...
    use shrike_syntax::{Did, Nsid, RecordKey, Tid};
