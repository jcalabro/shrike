...
fuzz_target!(|data: &[u8]| {
    let (roots, blocks) = match shrike_car::read_all(data) {
        Ok(r) => r,
        Err(_) => return,
    };

    // Re-write the parsed blocks.
    let written = shrike_car::write_all(&roots, &blocks)
        .expect("writing successfully-read blocks must not fail");

    // Re-read from the written bytes.
    let (roots2, blocks2) = shrike_car::read_all(&written[..])
    ...
    let written2 = shrike_car::write_all(&roots2, &blocks2)
