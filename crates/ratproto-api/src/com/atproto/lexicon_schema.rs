...
impl LexiconSchema {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), ratproto_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            ratproto_cbor::Encoder::new(&mut *buf).encode_text("lexicon")?;
            ratproto_cbor::Encoder::new(&mut *buf).encode_i64(self.lexicon)?;
        } else {
            // Slow path: merge known fields with extra_cbor, sort, encode.
            let mut pairs: Vec<(&str, Vec<u8>)> = Vec::new();
            {
                let mut vbuf = Vec::new();
                ratproto_cbor::Encoder::new(&mut vbuf).encode_i64(self.lexicon)?;
    ...
    pub fn decode_cbor(
        ...
        Ok(LexiconSchema {
            lexicon: field_lexicon.ok_or_else(|| {
                ratproto_cbor::CborError::InvalidCbor("missing required field 'lexicon'".into())
            })?,
