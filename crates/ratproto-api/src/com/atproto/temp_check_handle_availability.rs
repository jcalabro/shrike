...
) -> Result<TempCheckHandleAvailabilityOutput, ratproto_xrpc::Error> {
    client
...
impl TempCheckHandleAvailabilityResultUnavailable {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), ratproto_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            let count = 1u64;
            ratproto_cbor::Encoder::new(&mut *buf).encode_map_header(count)?;
            ratproto_cbor::Encoder::new(&mut *buf).encode_text("suggestions")?;
...
impl TempCheckHandleAvailabilitySuggestion {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), ratproto_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            ratproto_cbor::Encoder::new(&mut *buf).encode_text("method")?;
            ratproto_cbor::Encoder::new(&mut *buf).encode_text(&self.method)?;
        } else {
            // Slow path: merge known fields with extra_cbor, sort, encode.
            let mut pairs: Vec<(&str, Vec<u8>)> = Vec::new();
            {
            ...
            {
                let mut vbuf = Vec::new();
                ratproto_cbor::Encoder::new(&mut vbuf).encode_text(&self.method)?;
    ...
    pub fn decode_cbor(
        ...
        Ok(TempCheckHandleAvailabilitySuggestion {
            ...
            method: field_method.ok_or_else(|| {
                ratproto_cbor::CborError::InvalidCbor("missing required field 'method'".into())
            })?,
            extra: std::collections::HashMap::new(),
            extra_cbor,
        })
