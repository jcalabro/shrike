...
impl HostingGetAccountHistoryEmailConfirmed {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), ratproto_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            ratproto_cbor::Encoder::new(&mut *buf).encode_map_header(count)?;
            ratproto_cbor::Encoder::new(&mut *buf).encode_text("email")?;
            ratproto_cbor::Encoder::new(&mut *buf).encode_text(&self.email)?;
        } else {
            // Slow path: merge known fields with extra_cbor, sort, encode.
            let mut pairs: Vec<(&str, Vec<u8>)> = Vec::new();
            {
                let mut vbuf = Vec::new();
                ratproto_cbor::Encoder::new(&mut vbuf).encode_text(&self.email)?;
                pairs.push(("email", vbuf));
            }
            for (k, v) in &self.extra_cbor {
                pairs.push((k.as_str(), v.clone()));
    ...
    pub fn decode_cbor(
        ...
        Ok(HostingGetAccountHistoryEmailConfirmed {
            email: field_email.ok_or_else(|| {
                ratproto_cbor::CborError::InvalidCbor("missing required field 'email'".into())
            })?,
            extra: std::collections::HashMap::new(),
            extra_cbor,
        })
    ...
    pub fn decode_cbor(
        ...
        Ok(HostingGetAccountHistoryEvent {
            details: field_details.ok_or_else(|| {
                ratproto_cbor::CborError::InvalidCbor("missing required field 'details'".into())
            })?,
...
) -> Result<HostingGetAccountHistoryOutput, ratproto_xrpc::Error> {
    client
