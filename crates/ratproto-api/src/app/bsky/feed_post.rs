...
impl FeedPost {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), ratproto_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            if !self.entities.is_empty() {
                ratproto_cbor::Encoder::new(&mut *buf).encode_text("entities")?;
                ratproto_cbor::Encoder::new(&mut *buf)
...
impl FeedPostTextSlice {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), ratproto_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            let count = 2u64;
            ratproto_cbor::Encoder::new(&mut *buf).encode_map_header(count)?;
            ratproto_cbor::Encoder::new(&mut *buf).encode_text("end")?;
            ratproto_cbor::Encoder::new(&mut *buf).encode_i64(self.end)?;
            ratproto_cbor::Encoder::new(&mut *buf).encode_text("start")?;
            ratproto_cbor::Encoder::new(&mut *buf).encode_i64(self.start)?;
        } else {
            // Slow path: merge known fields with extra_cbor, sort, encode.
            let mut pairs: Vec<(&str, Vec<u8>)> = Vec::new();
            {
                let mut vbuf = Vec::new();
                ratproto_cbor::Encoder::new(&mut vbuf).encode_i64(self.end)?;
                pairs.push(("end", vbuf));
            }
            {
                let mut vbuf = Vec::new();
                ratproto_cbor::Encoder::new(&mut vbuf).encode_i64(self.start)?;
    ...
    pub fn decode_cbor(
        ...
        Ok(FeedPostTextSlice {
                ...
                ratproto_cbor::CborError::InvalidCbor("missing required field 'end'".into())
            })?,
            start: field_start.ok_or_else(|| {
                ratproto_cbor::CborError::InvalidCbor("missing required field 'start'".into())
            })?,
            extra: std::collections::HashMap::new(),
            extra_cbor,
        })
