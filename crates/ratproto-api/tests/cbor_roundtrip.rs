...
fn strong_ref_cbor_roundtrip() {
    let sr = ratproto_api::com::atproto::RepoStrongRef {
    ...
    let decoded = ratproto_api::com::atproto::RepoStrongRef::from_cbor(&cbor).unwrap();
...
fn strong_ref_cbor_key_order() {
    ...
    let val = ratproto_cbor::decode(&cbor).unwrap();
    if let ratproto_cbor::Value::Map(entries) = val {
...
fn strong_ref_preserves_extra_cbor() {
    ...
    let mut sr = ratproto_api::com::atproto::RepoStrongRef {
        uri: ratproto_syntax::AtUri::try_from("at://did:plc:abc/app.bsky.feed.post/123").unwrap(),
        cid: "bafyrei1234567890".into(),
        extra: Default::default(),
        extra_cbor: Vec::new(),
    ...
    let extra_value = ratproto_cbor::encode_value(&ratproto_cbor::Value::Unsigned(42)).unwrap();
    ...
    let extra_decoded = ratproto_cbor::decode(&decoded.extra_cbor[0].1).unwrap();
    assert_eq!(extra_decoded, ratproto_cbor::Value::Unsigned(42));
}

#[test]
fn feed_post_reply_ref_roundtrip() {
    let reply = ratproto_api::app::bsky::FeedPostReplyRef {
        parent: ratproto_api::com::atproto::RepoStrongRef {
            uri: ratproto_syntax::AtUri::try_from("at://did:plc:parent/app.bsky.feed.post/1")
                .unwrap(),
            cid: "bafyrei_parent".into(),
            extra: Default::default(),
            extra_cbor: Vec::new(),
        },
        root: ratproto_api::com::atproto::RepoStrongRef {
            uri: ratproto_syntax::AtUri::try_from("at://did:plc:root/app.bsky.feed.post/0")
                .unwrap(),
    ...
    let decoded = ratproto_api::app::bsky::FeedPostReplyRef::from_cbor(&cbor).unwrap();
...
fn feed_post_text_slice_roundtrip() {
    let slice = ratproto_api::app::bsky::FeedPostTextSlice {
        start: 0,
        end: 42,
        extra: Default::default(),
        extra_cbor: Vec::new(),
    ...
    let decoded = ratproto_api::app::bsky::FeedPostTextSlice::from_cbor(&cbor).unwrap();
...
fn embed_external_roundtrip() {
    let ext = ratproto_api::app::bsky::EmbedExternalExternal {
    ...
    let decoded = ratproto_api::app::bsky::EmbedExternalExternal::from_cbor(&cbor).unwrap();
...
fn feed_post_minimal_roundtrip() {
    let post = ratproto_api::app::bsky::FeedPost {
        text: "Hello world!".into(),
        created_at: ratproto_syntax::Datetime::try_from("2024-01-01T00:00:00.000Z").unwrap(),
        embed: None,
        entities: Vec::new(),
        facets: Vec::new(),
        labels: None,
    ...
    let decoded = ratproto_api::app::bsky::FeedPost::from_cbor(&cbor).unwrap();
...
fn feed_post_with_langs_roundtrip() {
    let post = ratproto_api::app::bsky::FeedPost {
        ...
        labels: None,
        langs: vec![
            ratproto_syntax::Language::try_from("en").unwrap(),
            ratproto_syntax::Language::try_from("ja").unwrap(),
        ],
        reply: None,
        tags: vec!["test".into()],
        extra: Default::default(),
