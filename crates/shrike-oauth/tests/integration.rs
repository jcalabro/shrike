//! Integration tests for shrike-oauth using a mock OAuth server.
...
async fn seed_auth_state(state_store: &dyn StateStore, base_url: &str) -> (String, String) {
    let dpop_key = shrike_crypto::P256SigningKey::generate();
    let pkce = shrike_oauth::pkce::generate_pkce();
...
async fn dpop_proof_has_correct_structure() {
    ...
    let jwt = shrike_oauth::dpop::create_dpop_proof(
        &key,
        "POST",
