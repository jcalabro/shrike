...
impl Session {
    ...
    pub fn dpop_key(&self) -> Result<shrike_crypto::P256SigningKey, OAuthError> {
        ...
        shrike_crypto::P256SigningKey::from_bytes(&arr)
            .map_err(|e| OAuthError::Crypto(e.to_string()))
    }

    /// Create a session from a signing key and token set.
    pub fn from_key_and_tokens(key: &shrike_crypto::P256SigningKey, token_set: TokenSet) -> Self {
        Session {
...
mod tests {
    ...
    fn make_session() -> Session {
        let key = shrike_crypto::P256SigningKey::generate();
    ...
    fn session_key_roundtrip() {
        ...
        let recovered = recovered.unwrap_or_else(|_| shrike_crypto::P256SigningKey::generate());
