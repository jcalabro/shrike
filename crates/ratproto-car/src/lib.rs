...
mod tests {
    ...
    where
        F: FnOnce(&mut ratproto_cbor::Encoder<&mut Vec<u8>>),
    {
        let mut header_buf = Vec::new();
        {
            let mut enc = ratproto_cbor::Encoder::new(&mut header_buf);
            build_fn(&mut enc);
        }
        let mut car_buf = Vec::new();
        ratproto_cbor::varint::encode_varint(header_buf.len() as u64, &mut car_buf);
    ...
    fn reader_truncated_block_errors() {
        ...
        ratproto_cbor::varint::encode_varint(200, &mut car);
    ...
    fn reader_block_with_wrong_cid_length_errors() {
        ...
        ratproto_cbor::varint::encode_varint(fake_data.len() as u64, &mut car);
