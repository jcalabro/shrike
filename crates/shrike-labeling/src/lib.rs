use shrike_cbor::{CborError, Cid, Encoder};
use shrike_crypto::{CryptoError, Signature, SigningKey, VerifyingKey};
use shrike_syntax::{Datetime, Did};
...
pub fn decode_label(data: &[u8]) -> Result<Label, LabelError> {
    let value = shrike_cbor::decode(data)?;
...
mod tests {
    ...
    fn make_full_label() -> Label {
        Label {
            src: Did::try_from("did:plc:labeler12345678901234").unwrap(),
            uri: "at://did:plc:user1234567890123456/app.bsky.feed.post/abc".into(),
            cid: Some(Cid::compute(shrike_cbor::Codec::Drisl, b"some-content")),
            val: "nudity".into(),
            neg: false,
    ...
    #[test]
    fn label_with_cid() {
        let cid = Cid::compute(shrike_cbor::Codec::Drisl, b"test");
        let label = Label {
            src: Did::try_from("did:plc:labeler12345678901234").unwrap(),
            uri: "at://did:plc:user1234567890123456/app.bsky.feed.post/abc".into(),
            cid: Some(cid),
    ...
    fn encode_decode_label_with_all_optional_fields() {
        let cid = Cid::compute(shrike_cbor::Codec::Drisl, b"some-content");
    ...
    fn encode_decode_label_with_only_cid_set() {
        let cid = Cid::compute(shrike_cbor::Codec::Raw, b"raw-content");
        let label = Label {
            src: Did::try_from("did:plc:labeler12345678901234").unwrap(),
            uri: "at://did:plc:user1234567890123456/app.bsky.feed.post/abc".into(),
            cid: Some(cid),
    ...
    fn roundtrip_preserves_all_field_values_exactly() {
        let cid = Cid::compute(shrike_cbor::Codec::Drisl, b"exact-content");
    ...
    fn decode_label_with_non_map_cbor_fails() {
        ...
        let mut buf = Vec::new();
        let mut enc = shrike_cbor::Encoder::new(&mut buf);
