...
impl RichtextFacetByteSlice {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), shrike_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            shrike_cbor::Encoder::new(&mut *buf).encode_text("byteEnd")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_i64(self.byte_end)?;
            shrike_cbor::Encoder::new(&mut *buf).encode_text("byteStart")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_i64(self.byte_start)?;
        } else {
            // Slow path: merge known fields with extra_cbor, sort, encode.
            let mut pairs: Vec<(&str, Vec<u8>)> = Vec::new();
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_i64(self.byte_end)?;
                pairs.push(("byteEnd", vbuf));
            }
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_i64(self.byte_start)?;
    ...
    pub fn decode_cbor(decoder: &mut shrike_cbor::Decoder) -> Result<Self, shrike_cbor::CborError> {
        ...
        Ok(RichtextFacetByteSlice {
            byte_end: field_byte_end.ok_or_else(|| {
                shrike_cbor::CborError::InvalidCbor("missing required field 'byteEnd'".into())
            })?,
            byte_start: field_byte_start.ok_or_else(|| {
                shrike_cbor::CborError::InvalidCbor("missing required field 'byteStart'".into())
            })?,
...
impl RichtextFacet {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), shrike_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            shrike_cbor::Encoder::new(&mut *buf).encode_text("features")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_array_header(self.features.len() as u64)?;
