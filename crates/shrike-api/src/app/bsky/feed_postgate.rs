...
pub struct FeedPostgate {
    ...
    pub detached_embedding_uris: Vec<shrike_syntax::AtUri>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
impl FeedPostgate {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), shrike_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            if !self.embedding_rules.is_empty() {
                shrike_cbor::Encoder::new(&mut *buf).encode_text("embeddingRules")?;
            ...
            if !self.detached_embedding_uris.is_empty() {
                shrike_cbor::Encoder::new(&mut *buf).encode_text("detachedEmbeddingUris")?;
                shrike_cbor::Encoder::new(&mut *buf)
    ...
    pub fn decode_cbor(decoder: &mut shrike_cbor::Decoder) -> Result<Self, shrike_cbor::CborError> {
        ...
        let mut field_detached_embedding_uris: Vec<shrike_syntax::AtUri> = Vec::new();
        let mut extra_cbor: Vec<(String, Vec<u8>)> = Vec::new();
