...
impl EmbedRecordViewDetached {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), shrike_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            shrike_cbor::Encoder::new(&mut *buf).encode_text("uri")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_text(self.uri.as_str())?;
            shrike_cbor::Encoder::new(&mut *buf).encode_text("detached")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_bool(self.detached)?;
        } else {
            // Slow path: merge known fields with extra_cbor, sort, encode.
            let mut pairs: Vec<(&str, Vec<u8>)> = Vec::new();
            {
            ...
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_bool(self.detached)?;
    ...
    pub fn decode_cbor(decoder: &mut shrike_cbor::Decoder) -> Result<Self, shrike_cbor::CborError> {
        ...
        Ok(EmbedRecordViewDetached {
            ...
            detached: field_detached.ok_or_else(|| {
                shrike_cbor::CborError::InvalidCbor("missing required field 'detached'".into())
            })?,
            extra: std::collections::HashMap::new(),
            extra_cbor,
        })
...
impl EmbedRecordViewRecord {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), shrike_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            self.author.encode_cbor(buf)?;
            if !self.embeds.is_empty() {
                shrike_cbor::Encoder::new(&mut *buf).encode_text("embeds")?;
