...
) -> Result<ModerationGetActorMetadataOutput, shrike_xrpc::Error> {
    client
...
impl ModerationGetActorMetadataMetadata {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), shrike_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            shrike_cbor::Encoder::new(&mut *buf).encode_text("convos")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_i64(self.convos)?;
            shrike_cbor::Encoder::new(&mut *buf).encode_text("messagesSent")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_i64(self.messages_sent)?;
            shrike_cbor::Encoder::new(&mut *buf).encode_text("convosStarted")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_i64(self.convos_started)?;
            shrike_cbor::Encoder::new(&mut *buf).encode_text("messagesReceived")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_i64(self.messages_received)?;
        } else {
            // Slow path: merge known fields with extra_cbor, sort, encode.
            let mut pairs: Vec<(&str, Vec<u8>)> = Vec::new();
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_i64(self.convos)?;
                pairs.push(("convos", vbuf));
            }
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_i64(self.messages_sent)?;
                pairs.push(("messagesSent", vbuf));
            }
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_i64(self.convos_started)?;
                pairs.push(("convosStarted", vbuf));
            }
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_i64(self.messages_received)?;
