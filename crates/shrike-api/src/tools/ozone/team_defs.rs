...
impl TeamDefsMember {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), shrike_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            shrike_cbor::Encoder::new(&mut *buf).encode_text("role")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_text(&self.role)?;
            if self.profile.is_some() {
            ...
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_text(&self.role)?;
    ...
    pub fn decode_cbor(decoder: &mut shrike_cbor::Decoder) -> Result<Self, shrike_cbor::CborError> {
        ...
        Ok(TeamDefsMember {
            ...
            role: field_role.ok_or_else(|| {
                shrike_cbor::CborError::InvalidCbor("missing required field 'role'".into())
            })?,
