...
impl SyncListHostsHost {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), shrike_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            shrike_cbor::Encoder::new(&mut *buf).encode_text("hostname")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_text(&self.hostname)?;
            if self.account_count.is_some() {
                shrike_cbor::Encoder::new(&mut *buf).encode_text("accountCount")?;
            ...
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_text(&self.hostname)?;
    ...
    pub fn decode_cbor(decoder: &mut shrike_cbor::Decoder) -> Result<Self, shrike_cbor::CborError> {
        ...
        Ok(SyncListHostsHost {
            ...
            hostname: field_hostname.ok_or_else(|| {
                shrike_cbor::CborError::InvalidCbor("missing required field 'hostname'".into())
            })?,
...
) -> Result<SyncListHostsOutput, shrike_xrpc::Error> {
