...
) -> Result<SyncListReposOutput, shrike_xrpc::Error> {
...
impl SyncListReposRepo {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), shrike_cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            shrike_cbor::Encoder::new(&mut *buf).encode_text("head")?;
            shrike_cbor::Encoder::new(&mut *buf).encode_text(&self.head)?;
            if self.active.is_some() {
            ...
            {
                let mut vbuf = Vec::new();
                shrike_cbor::Encoder::new(&mut vbuf).encode_text(&self.head)?;
    ...
    pub fn decode_cbor(
        ...
        Ok(SyncListReposRepo {
            ...
            head: field_head.ok_or_else(|| {
                shrike_cbor::CborError::InvalidCbor("missing required field 'head'".into())
            })?,
            active: field_active,
            status: field_status,
