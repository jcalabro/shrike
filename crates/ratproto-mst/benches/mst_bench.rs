...
use ratproto_mst::{BlockStore, MemBlockStore, MstError, Tree, height_for_key};
...
fn bench_node_codec(c: &mut Criterion) {
    ...
    group.bench_function("decode_node", |b| {
        b.iter(|| {
            let nd = ratproto_mst::node::decode_node_data(black_box(&root_block)).expect("decode");
            black_box(nd);
        });
    });

    let nd = ratproto_mst::node::decode_node_data(&root_block).expect("decode");
    group.bench_function("encode_node", |b| {
        b.iter(|| {
            let encoded = ratproto_mst::node::encode_node_data(black_box(&nd)).expect("encode");
            black_box(encoded);
        });
    });
