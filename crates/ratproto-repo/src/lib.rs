...
pub enum RepoError {
    ...
    Commit(String),
    #[error("MST error: {0}")]
    Mst(#[from] ratproto_mst::MstError),
    #[error("CBOR error: {0}")]
    Cbor(#[from] ratproto_cbor::CborError),
    #[error("crypto error: {0}")]
    Crypto(#[from] ratproto_crypto::CryptoError),
}
...
mod tests {
    use ratproto_cbor::{Cid, Codec, Encoder, Value};
    use ratproto_crypto::SigningKey;
    ...
    fn v2_commit_decodes_without_rev_and_sig() {
        ...
        let encoded = ratproto_cbor::encode_value(&val).unwrap();
