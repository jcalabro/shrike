use ratproto_cbor::{Cid, Encoder, Value, decode, encode_text_map};
use ratproto_crypto::{Signature, SigningKey, VerifyingKey};
use ratproto_syntax::{Did, Tid};
...
impl Commit {
    ...
    fn encode_field<W: std::io::Write>(
        ...
        enc: &mut Encoder<W>,
        key: &str,
    ) -> Result<(), ratproto_cbor::CborError> {
        match key {
