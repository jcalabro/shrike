use ratproto_cbor::{CborError, Cid, Encoder};
use ratproto_crypto::{CryptoError, Signature, SigningKey, VerifyingKey};
use ratproto_syntax::{Datetime, Did};
...
pub fn decode_label(data: &[u8]) -> Result<Label, LabelError> {
    let value = ratproto_cbor::decode(data)?;
...
mod tests {
    ...
    fn make_full_label() -> Label {
        Label {
            src: Did::try_from("did:plc:labeler12345678901234").unwrap(),
            uri: "at://did:plc:user1234567890123456/app.bsky.feed.post/abc".into(),
            cid: Some(Cid::compute(ratproto_cbor::Codec::Drisl, b"some-content")),
            val: "nudity".into(),
            neg: false,
    ...
    fn sign_and_verify_label() {
        let sk = ratproto_crypto::P256SigningKey::generate();
        let mut label = Label {
            src: Did::try_from("did:plc:labeler12345678901234").unwrap(),
            uri: "at://did:plc:user1234567890123456/app.bsky.feed.post/abc".into(),
            cid: None,
    ...
    #[test]
    fn label_with_cid() {
        let cid = Cid::compute(ratproto_cbor::Codec::Drisl, b"test");
        let label = Label {
            src: Did::try_from("did:plc:labeler12345678901234").unwrap(),
            uri: "at://did:plc:user1234567890123456/app.bsky.feed.post/abc".into(),
            cid: Some(cid),
    ...
    fn sign_and_verify_with_k256_key() {
        let sk = ratproto_crypto::K256SigningKey::generate();
        let mut label = make_test_label();
        sign_label(&mut label, &sk).unwrap();
    ...
    fn verify_with_wrong_key_fails() {
        let sk1 = ratproto_crypto::P256SigningKey::generate();
        let sk2 = ratproto_crypto::P256SigningKey::generate();
        let mut label = make_test_label();
    ...
    fn encode_decode_label_with_all_optional_fields() {
        let cid = Cid::compute(ratproto_cbor::Codec::Drisl, b"some-content");
    ...
    fn encode_decode_label_with_only_cid_set() {
        let cid = Cid::compute(ratproto_cbor::Codec::Raw, b"raw-content");
        let label = Label {
            src: Did::try_from("did:plc:labeler12345678901234").unwrap(),
            uri: "at://did:plc:user1234567890123456/app.bsky.feed.post/abc".into(),
            cid: Some(cid),
    ...
    fn roundtrip_preserves_all_field_values_exactly() {
        let cid = Cid::compute(ratproto_cbor::Codec::Drisl, b"exact-content");
    ...
    fn decode_label_with_non_map_cbor_fails() {
        ...
        let mut buf = Vec::new();
        let mut enc = ratproto_cbor::Encoder::new(&mut buf);
