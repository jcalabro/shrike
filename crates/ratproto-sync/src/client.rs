...
pub struct SyncClient {
    ...
    #[allow(dead_code)]
    identity: Option<Arc<ratproto_identity::Directory>>,
}

impl SyncClient {
    /// Create a new `SyncClient` backed by an XRPC client without identity resolution.
    pub fn new(xrpc: ratproto_xrpc::Client) -> Self {
        SyncClient {
            xrpc,
            identity: None,
        }
    ...
    pub fn with_identity(
        xrpc: ratproto_xrpc::Client,
        dir: Arc<ratproto_identity::Directory>,
    ) -> Self {
        SyncClient {
            xrpc,
            identity: Some(dir),
    ...
    pub async fn get_repo(&self, did: &Did) -> Result<DownloadedRepo, SyncError> {
        ...
        let (roots, blocks) = ratproto_car::read_all(&car_bytes[..])?;
        ...
        let commit = ratproto_repo::Commit::from_cbor(&root_block.data)?;
...
mod tests {
    ...
    use ratproto_syntax::{Did, Nsid, RecordKey, TidClock};
    ...
    fn sync_client_construction() {
        let client = SyncClient::new(ratproto_xrpc::Client::new("https://bsky.social"));
    ...
    fn sync_client_with_identity_construction() {
        let dir = Arc::new(ratproto_identity::Directory::new());
        let client =
            SyncClient::with_identity(ratproto_xrpc::Client::new("https://bsky.social"), dir);
        let _ = client;
    }
    ...
    fn sync_client_with_custom_plc_url() {
        let dir = Arc::new(ratproto_identity::Directory::with_plc_url(
            "https://custom-plc.example.com",
        ));
        let client =
            SyncClient::with_identity(ratproto_xrpc::Client::new("https://pds.example.com"), dir);
        let _ = client;
    }

    #[test]
    fn sync_client_with_default_directory() {
        let dir = Arc::new(ratproto_identity::Directory::default());
        let client =
            SyncClient::with_identity(ratproto_xrpc::Client::new("https://bsky.social"), dir);
        let _ = client;
    }
