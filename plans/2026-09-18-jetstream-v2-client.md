...
Status: in progress; M0, M1, M2, M3, M4, M5 complete (M3 native complete + roasted/hardened; browser range/stream + CORS deferred to M6; M4 browser live headless testing deferred to M6, adapter compiles via wasm-check; M5 engine + independent-oracle model tests complete and roasted/hardened, native cancellation covered, WASM future/listener cleanup deferred to M6 headless testing). M6 nearly complete: CLI subcommand, browser archive HTTP transport, the `connectJetstreamV2` browser binding (with network-free headless node tests), the v2 demo UI, public rustdoc + README, and cursor-persistence docs are all complete and roasted. Remaining are the two items that require live network/credentials: automated headless-browser coverage against a local cross-origin fixture (line 601) and production live/snapshot smoke runs (line 605); both are exercised by hand via the demo and `just jetstream-smoke`. The final whole-branch re-roast is complete and converged across two confirmatory survey passes: five distinct confirmed findings (two HIGH, three MEDIUM) plus two LOW follow-ups, all resolved (behavioral fixes carry regression tests; documentation-accuracy fixes are noted as such), with every follow-up fix re-roasted clean (see "Final whole-branch re-roast" below).
...
M5 roast fixes: a post-commit roast raised three confirmed findings; the fix commit re-roasted clean. (1) `EngineConfig::validate` now rejects `before_seq` unless `snapshot_only` is set — an upper archive bound cannot be safely followed by an unbounded live tail (regression tests `rejects_before_seq_without_snapshot_only` + positive `before_seq_allowed_with_snapshot_only`). (2) `replay_archive` no longer silently drops a distinct event whose seq regresses below one already delivered from an earlier segment; it collapses only true duplicates (`== floor` / `<= after`) and rejects a genuine lower-seq regression as `Error::PlanInvalid`, since merging arbitrary segment overlap would require unbounded buffering (regression test `overlapping_segments_out_of_order_rejected`; the overclaiming doc comment was corrected). (3) The third finding (progress advanced "before" delivery) was adjudicated a false positive: `EngineSink` is accept-and-stop — the `Delivery` is moved into `deliver`, so the consumer has it by the time the call returns and `false` only halts future deliveries. The premature reorder was reverted, the contract was made explicit in the trait docs, and `stopping_batch_counted_in_progress` pins the archive-flush side of it.
...
- [x] Add a v2 live demo for `jetstream.us-east.bsky.network` to `wasm/index.html` and `wasm/README.md`, with optional bounded archive replay. (`wasm/index.html` gains a "Jetstream v2 · sealed archive + live" section defaulting to `jetstream.us-east.bsky.network` with optional `afterSeq`/`beforeSeq`/snapshot replay; `wasm/README.md` documents the `connectJetstreamV2` binding, its options, and the key-handling rule.)
- [x] Accept a replay key for the browser session without persisting it or adding it to the URL. Never embed a long-lived key in checked-in HTML, JavaScript, WASM, or examples. (The demo reads the key from a `type="password"` field, hands it to `connectJetstreamV2` once, then blanks the field; it is never written to storage or the URL, and no key is checked in.)
- [x] Show v2 sequence cursors, event kinds, replay/cutover progress, cancellation, clear errors, and compression fallback in the demo. (A live stats grid polls `subscription.stats()` for `lastProcessedSeq`/`sealedTipSeq`/`plannedThroughSeq`/`residualGap`/`deliveredEvents`/archive pages/active downloads; events render kind + seq + collection; `#info` advisories and stream errors surface in the log; the "disable compression" toggle drives the uncompressed fallback; Stop cancels the engine.)
- [ ] Test the v2 binding and UI in a local headless browser: live frames, bounded replay, CORS/header errors, cancellation, listener cleanup, and compression fallback. (Partial: automated `wasm-bindgen_test` coverage in `wasm/src/lib.rs` runs in the Node headless runner — not a real browser — and proves only the network-free binding contract: snapshot/`afterSeq` require a `replayKey`, unknown kinds are rejected, a cleartext archive key to a non-loopback host is refused, and a live-only subscription yields a zeroed camelCase `stats()` with an idempotent `close()`. The remaining browser-only coverage — live frames, bounded replay, CORS/header errors, and compression fallback exercised in an actual headless browser against a local cross-origin fixture (see the acceptance gates at lines 680/792/892) — is not yet automated; it is currently only exercised by hand through the demo and `just jetstream-smoke`.)
- [x] Add public rustdoc and a minimal README example. (`src/jetstream/mod.rs` replaces the milestone-status blurb with a public `# Overview`, a compiling `no_run` live-tail example (`cargo test --doc` passes), and a `# Cursors and resuming` section; `README.md` adds a `jetstream` row to the feature-module table linking the docs.rs page, and `wasm/README.md` carries the browser `connectJetstreamV2` example.)
- [x] Document cursor persistence with host identity, marker folding responsibility, snapshot semantics, resource knobs, and recoverable/fatal handling. (The `# Cursors and resuming` section in `src/jetstream/mod.rs` covers all five: cursors are per-host `u64` sequence numbers, callers fold the highest delivered `seq` after `deliver` returns, resuming from a non-zero `after_seq` requires an archive source (replay `(after_seq, tip]` then cut over once at `max(tip, last_processed_seq)`), `snapshot_only`/inclusive `before_seq` semantics, the `read_limit`/`max_batch`/backoff/`Limits`/`DEFAULT_MAX_REBACKFILL_STALLS` knobs, and that recoverable errors continue in order while a fatal error is only `Engine::run`'s `Err` return.)
...
M6 notes: shipped across several commits, each roasted after commit. `shrike jetstream` (`tools/shrike/src/jetstream.rs`) drives the native `Engine` in live / replay-cutover / snapshot modes with `--json`/`--stats` and an environment-only key; `just jetstream-smoke` is the never-in-CI production smoke recipe. The browser gains a wasm-gated `WasmHttpTransport` archive path and a `connectJetstreamV2` binding (`wasm/src/lib.rs`) wired only to `shrike::jetstream`, with the legacy `connectJetstream`/`connectFirehose` bindings untouched; five network-free `wasm-bindgen_test`s in the Node runner pin the binding contract (replay-key requirements, unknown-kind rejection, cleartext-key refusal, zeroed camelCase `stats()` with idempotent `close()`). `wasm/index.html` adds a "Jetstream v2 · sealed archive + live" demo section that reads the replay key from a session-only field and blanks it on start, renders cursors/kinds/progress/errors, and drives the compression toggle. Docs: `src/jetstream/mod.rs` now carries a public overview, a compiling `no_run` live-tail example, and a cursor-persistence section; `README.md` lists the `jetstream` feature; `wasm/README.md` documents the browser binding and browser constraints.

M6 roast fixes: the v2 demo's cursor inputs were parsed with `Number()`, which silently rounds integers above 2^53 and yields `NaN`/`Infinity` — a mistyped cursor would have been sent as a different value with no error; `parseSeq` now requires `^\d+$` and `Number.isSafeInteger`. The demo's password field is now cleared in a `finally` so a synchronous validation throw (e.g. a cleartext archive key to a non-loopback host) cannot leave the key in the DOM. The docs commit's overview mislabeled collection matching as an "NSID prefix" (the real `Filter` contract is an exact NSID or a terminal `.*` wildcard) and wrote the cutover resume point as "at `max(tip, last_processed_seq)`" without noting the live tail actually resumes at that boundary `+ 1`; both were corrected and the fix re-roasted clean.

M6 deferred (network/tooling-gated, not implementable here): automated headless-browser coverage against a local cross-origin fixture (line 601) and production live/snapshot smoke runs (line 605). Both require live network, a real browser test harness, and/or credentials, and are currently exercised by hand through the demo and `just jetstream-smoke`; the automated `wasm-bindgen_test` suite deliberately covers only the network-free binding paths.

Final whole-branch re-roast (survey mode, since the branch diff is too large for a diff roast and branch survey is disabled): three confirmed findings, all resolved. (1) R-fe03a6 [HIGH]: `ArchiveClient::new` interpolated the raw config host straight into the request URL, so a URL-shaped host such as `trusted.example@attacker.example` would send the bearer key to `attacker.example` (the real authority after `@`). The host is now normalized through `normalize_host` before the cleartext-key check and base-URL construction; the same fail-fast was added to `LiveConfig::validate` (defense-in-depth — no key rides the live/dict paths). (2) R-2a230c [HIGH, adjudicated to a documentation defect]: the download module doc claimed *every* segment is checksum-verified, but per this plan the xxh3 recompute is scoped to whole downloads (the hash covers only header+footer, and sparse `blocks` fetch exists precisely to avoid pulling the whole object). The doc was corrected to describe each mode's real integrity posture and the format-level caveat that no per-block content hash exists; blocks-mode behavior is spec-conformant and unchanged. (3) R-589dd4 [MEDIUM], caught by the diff roast of fix (1): `LiveConfig::validate` discarded the value `normalize_host` returned, so the raw host still reached `subscribe_url`/the dictionary source. Canonicalization moved to the URL-construction boundaries (`subscribe_url` and the extracted free fn `dictionary_url`), the single enforcement point for every caller; the fix re-roasted clean. All three carry regression tests that inspect the rejection/constructed URL, not merely that validation passed. A confirmatory second survey pass then found (4) R-6759cc [MEDIUM]: `NativeHttpTransport::with_client` was a public escape hatch that stored an arbitrary caller-supplied `reqwest::Client` unchanged, so a default (redirect-following) client could silently defeat the `redirect::Policy::none()` invariant `new` establishes and — with the bearer key attached by `ArchiveClient::authorized` — follow a same-host HTTPS→HTTP downgrade, leaking the key over cleartext (CWE-319). `with_client` had zero callers workspace-wide, so it was removed outright, making `new` the sole (redirect-disabled) construction path; a loopback regression test asserts a 302 is surfaced verbatim, not followed. The diff roast of that fix flagged (5) R-09917e [LOW]: the test inferred non-following from a hard-coded dead port (an external host-state assumption), so it was hardened to count connection hops via a self-referential 302 owned by the test — deterministic, race-free, and re-roasted clean. A second confirmatory survey then converged with one genuinely new finding: (6) R-5d984e [MEDIUM, CWE-400]: the browser (wasm) live WebSocket transport's module doc over-claimed that `read_limit` bounds the receive buffer, but the browser `WebSocket` API delivers each message fully buffered before `ws.next()` yields it and exposes no receive-size cap — so unlike native's tungstenite `max_message_size`, the limit can only bound downstream processing (reject before decompress/decode and tear down), not the transient buffer for one oversized message. This is a browser-platform capability limit (the same shape as the missing-CORS case), so it was resolved by correcting the doc and adding an inline note at the check site; behavior is unchanged and correct. Its diff roast flagged (7) R-908da0 [LOW]: the new doc said "frame" where the check applies to a complete WebSocket message — corrected and re-roasted clean. The same survey also re-surfaced R-cfa113 [MEDIUM] (blocks-mode per-block authenticity), which is the already-adjudicated R-2a230c — the verifier confirmed the whole-download "fix" is not a remedy and that it is a documented, spec-conformant format trust-model property requiring a compromised first-party origin — and refuted R-67bc9e [HIGH] (a claimed stripe-copy panic), since stripe request bounds come from `compute_stripes`, clamped to `start <= end < total`, not from the server's `Content-Range`.

### Go-behavioral-parity pass (2026-09-19)

A follow-up review compared this client's *observable behavior* against the pinned Go reference (`bluesky-social/jetstream` @ `58c4d7f7`, with the atmos Go library as the JSON→CBOR reference) with the explicit goal of exact behavioral parity, and the divergences were implemented. The user locked three scope decisions up front: keep the validated public event types (Did/Nsid/RecordKey/Tid) but surface dropped rows/frames as ordered recoverable errors; keep the corrupt-input hardening (internal xxh3 self-verify + container validation) but drop the plan-checksum *equality* binding — a re-plan is a new generation, not an integrity failure — and *add* Go's duplicate-JSON-key rejection; and restructure the engine to interleave planning and download page-by-page like Go, with Go's Stats timing.

The parity fixes, all carrying regression tests: (1) **Compression** — `decompress_bounded` now mirrors klauspost `DecodeAll` exactly: multi-frame concatenation, skippable frames, `maxWindowSize = 1<<29` clamped by the caller's memory bound, trailing-garbage rejection, and empty-input→empty-output. (2) **Cursor resume is exclusive** — `Cursor::Seq`/`LiveCursor::Resume` resume *after* the given seq (dedup floor = cursor; the server replays inclusively and the client dedups), matching Go's `WithLiveCursor`; `Seq(0)` is now legal ("everything"), and the engine cutover drops the old `+1`. (3) **Retry** — `retryableStatus` is `429|5xx` only; `parseRetryAfter` follows Go's precedence (RateLimit-Reset first with the `until==0` fallthrough quirk, IMF-fixdate support, 30s clamp as a floor); 408 is terminal; the generation-restart budget is bounded; backoff sleeps race cancellation. (4) **ReadFull stripe semantics** — a short stripe body is a retryable truncation, surplus is discarded, and a missing stripe ETag is treated as a generation change. (5) **Live tail** — malformed frames and zstd failures are reported as ordered recoverable errors with the connection *kept*; a "live tail reconnecting" recoverable notice precedes every backoff-paced redial (including after a dictionary refresh, which does *not* reset backoff); the 20ms flush is a free-running max-delay ticker, not a debounce; an empty subprotocol echo is accepted (native redials once without offering the subprotocol, working around tungstenite's stricter-than-RFC handshake). (6) **Engine** — planning and download interleave page-by-page: the sealed tip is published before download, pages/coverage after emit; a per-entry download failure is an ordered recoverable error and the sweep continues; a re-backfill after `CursorTooOld` never rewinds below the delivered live cursor (a `seen` watermark drives resume), and a re-planned straddling segment's already-delivered rows are collapsed without a spurious `PlanInvalid`. (7) **Strict grammars** — RFC-3339 parsing rejects the jiff extensions Go's `time.RFC3339Nano` would reject; a commit record's raw wire text is rejected for duplicate JSON object keys (serde_json's tree silently resolves duplicates last-wins, rewriting the canonical bytes). (8) **CLI** — `--host` is required (no default, so an accidental invocation cannot start a production full replay); stdout writes are broken-pipe-safe (a closed downstream reader is a clean stop, like Go's `enc.Encode` error path); a second Ctrl-C force-exits a run wedged mid-batch.

Deliberately *not* changed (flagged as still-open, non-parity review findings, all at the wasm/JS boundary): the browser binding's README ES-Maps mismatch, the JS error-class distinction, and the browser completion callback. These are browser-platform ergonomics, not behavioral divergences from the Go client.
