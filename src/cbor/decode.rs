...
mod tests {
    use super::*;
    use crate::cbor::{Codec, encode_value};
    ...
    fn reject_indefinite_length() {
        assert!(crate::cbor::decode(&[0x5f]).is_err()); // indefinite bytes
        assert!(crate::cbor::decode(&[0x7f]).is_err()); // indefinite text
        assert!(crate::cbor::decode(&[0x9f]).is_err()); // indefinite array
        assert!(crate::cbor::decode(&[0xbf]).is_err()); // indefinite map
    }

    #[test]
    fn reject_non_minimal_int() {
        // 23 encoded with additional info 24 (should be inline)
        assert!(crate::cbor::decode(&[0x18, 0x17]).is_err());
        // 255 encoded with additional info 25 (should use 1-byte form)
        assert!(crate::cbor::decode(&[0x19, 0x00, 0xff]).is_err());
    }
    ...
    fn reject_half_float() {
        assert!(crate::cbor::decode(&[0xf9, 0x00, 0x00]).is_err());
    }

    #[test]
    fn reject_single_float() {
        assert!(crate::cbor::decode(&[0xfa, 0x00, 0x00, 0x00, 0x00]).is_err());
    }

    #[test]
    fn reject_nan() {
        assert!(
            crate::cbor::decode(&[0xfb, 0x7f, 0xf8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).is_err()
        );
    }
    ...
    fn reject_infinity() {
        // +Infinity
        assert!(
            crate::cbor::decode(&[0xfb, 0x7f, 0xf0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).is_err()
        );
        // -Infinity
        assert!(
            crate::cbor::decode(&[0xfb, 0xff, 0xf0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).is_err()
        );
    }
    ...
    fn reject_tag_not_42() {
        // Tag 1 (datetime)
        assert!(crate::cbor::decode(&[0xc1, 0x00]).is_err());
    }
    ...
    fn roundtrip_complex() {
        ...
        let decoded = crate::cbor::decode(&encoded).unwrap();
    ...
    fn reject_deeply_nested_arrays() {
        ...
        let result = crate::cbor::decode(&buf);
    ...
    fn accept_moderate_nesting() {
        ...
        assert!(crate::cbor::decode(&buf).is_ok());
    }
