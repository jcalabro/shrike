                        ...
                        for item in items {
                            match item {
                                crate::cbor::Value::Unsigned(n) => {
                                    field_ids.push(i64::try_from(n).map_err(|_| {
...
#[derive(Debug)]
pub struct ModerationEmitEventReportActionCborView<'a> {
    pub all: Option<bool>,
    pub ids: Vec<i64>,
...
impl<'a> ModerationEmitEventReportActionCborView<'a> {
    #[inline]
    ...
    pub fn to_owned(&self) -> Result<ModerationEmitEventReportAction, crate::cbor::CborError> {
        Ok(ModerationEmitEventReportAction {
            all: self.all,
            ids: self.ids.clone(),
            note: self.note.as_ref().map(|value| (*value).to_owned()),
    ...
    pub fn decode_cbor(
        ...
        let mut field_all: Option<bool> = None;
        let mut field_ids: Vec<i64> = Vec::new();
        let mut field_note: Option<&'a str> = None;
        let mut field_types: Vec<String> = Vec::new();
        let mut extra_cbor = Vec::new();
        let mut entries = decoder.map_entries()?;
        entries.try_field(b"\x63\x61\x6c\x6c", |decoder| {
            let value = decoder.decode()?;
            if let crate::cbor::Value::Bool(b) = value {
                field_all = Some(b);
            } else {
        ...
        entries.try_field(b"\x63\x69\x64\x73", |decoder| {
            let value = decoder.decode()?;
            if let crate::cbor::Value::Array(items) = value {
                for item in items {
                    match item {
        ...
        entries.try_field(b"\x64\x6e\x6f\x74\x65", |decoder| {
            field_note = Some(decoder.text()?);
            Ok(())
        })?;
        entries.try_field(b"\x65\x74\x79\x70\x65\x73", |decoder| {
            let value = decoder.decode()?;
            if let crate::cbor::Value::Array(items) = value {
                for item in items {
                    if let crate::cbor::Value::Text(s) = item {
