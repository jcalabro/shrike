            ...
            match key {
                ...
                "uses" => {
                    ...
                    if let crate::cbor::Value::Array(items) = value {
                        for item in items {
                            field_uses.push(ServerDefsInviteCodeUse::from_cbor_value(item)?);
                        }
                    } else {
                ...
                "available" => {
                    ...
                    match value {
                        crate::cbor::Value::Unsigned(n) => {
                            field_available = Some(i64::try_from(n).map_err(|_| {
...
#[derive(Debug)]
pub struct ServerDefsInviteCodeCborView<'a> {
    pub code: &'a str,
    pub uses: Vec<ServerDefsInviteCodeUse>,
    pub disabled: bool,
    pub available: i64,
    ...
    pub for_account: &'a str,
    pub extra_cbor: Vec<(&'a str, &'a [u8])>,
    raw_cbor: &'a [u8],
}
impl<'a> ServerDefsInviteCodeCborView<'a> {
    #[inline]
    ...
    pub fn to_owned(&self) -> Result<ServerDefsInviteCode, crate::cbor::CborError> {
        Ok(ServerDefsInviteCode {
            ...
            for_account: self.for_account.to_owned(),
            extra: std::collections::HashMap::new(),
            extra_cbor: self
                .extra_cbor
                .iter()
    ...
    pub fn decode_cbor(
        ...
        let mut field_for_account: Option<&'a str> = None;
        ...
        entries.try_field(b"\x64\x75\x73\x65\x73", |decoder| {
            let value = decoder.decode()?;
            if let crate::cbor::Value::Array(items) = value {
                for item in items {
                    field_uses.push(ServerDefsInviteCodeUse::from_cbor_value(item)?);
        ...
        entries.try_field(b"\x69\x61\x76\x61\x69\x6c\x61\x62\x6c\x65", |decoder| {
            let value = decoder.decode()?;
            match value {
                crate::cbor::Value::Unsigned(n) => {
                    field_available = Some(i64::try_from(n).map_err(|_| {
        ...
        entries.try_field(b"\x6a\x66\x6f\x72\x41\x63\x63\x6f\x75\x6e\x74", |decoder| {
            field_for_account = Some(decoder.text()?);
            Ok(())
        })?;
...
/// A validated CBOR view. Borrowed fields cannot outlive the input.
#[derive(Debug)]
pub struct ServerDefsInviteCodeUseCborView<'a> {
    pub used_at: crate::syntax::DatetimeRef<'a>,
    pub used_by: crate::syntax::Did,
    pub extra_cbor: Vec<(&'a str, &'a [u8])>,
    raw_cbor: &'a [u8],
}
impl<'a> ServerDefsInviteCodeUseCborView<'a> {
    #[inline]
    pub fn from_cbor(data: &'a [u8]) -> Result<Self, crate::cbor::CborError> {
        let mut decoder = crate::cbor::Decoder::new(data);
        let result = Self::decode_cbor(&mut decoder)?;
    ...
    pub fn to_owned(&self) -> Result<ServerDefsInviteCodeUse, crate::cbor::CborError> {
        Ok(ServerDefsInviteCodeUse {
            used_at: self.used_at.to_owned(),
    ...
    pub fn decode_cbor(
        ...
        let start = decoder.position();
        let mut field_used_at: Option<crate::syntax::DatetimeRef<'a>> = None;
        let mut field_used_by: Option<crate::syntax::Did> = None;
        let mut extra_cbor = Vec::new();
        let mut entries = decoder.map_entries()?;
        entries.try_field(b"\x66\x75\x73\x65\x64\x41\x74", |decoder| {
            field_used_at = Some(
                crate::syntax::DatetimeRef::try_from(decoder.text()?)
                    .map_err(|e| crate::cbor::CborError::InvalidCbor(e.to_string()))?,
            );
        ...
        entries.try_field(b"\x66\x75\x73\x65\x64\x42\x79", |decoder| {
            let value = decoder.decode()?;
            if let crate::cbor::Value::Text(s) = value {
                field_used_by = Some(
                    crate::syntax::Did::try_from(s)
