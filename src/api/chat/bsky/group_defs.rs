...
/// GroupDefsDisabledJoinLinkPreviewView — Preview for a disabled join link. Carries only the code so clients can correlate with the input and render a disabled state.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GroupDefsDisabledJoinLinkPreviewView {
    pub code: String,
...
impl GroupDefsDisabledJoinLinkPreviewView {
...
/// GroupDefsInvalidJoinLinkPreviewView — Preview for a join link code that does not map to an existing link. Carries only the code so clients can correlate with the input and render an invalid state.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GroupDefsInvalidJoinLinkPreviewView {
    pub code: String,
    /// Extra fields not defined in the schema (JSON).
    #[serde(flatten)]
    pub extra: std::collections::HashMap<String, serde_json::Value>,
...
impl GroupDefsInvalidJoinLinkPreviewView {
    pub fn to_cbor(&self) -> Result<Vec<u8>, crate::cbor::CborError> {
        let mut buf = Vec::new();
        self.encode_cbor(&mut buf)?;
        Ok(buf)
...
/// GroupDefsJoinLinkPreviewView — Preview that can be shown in feeds, including to unauthenticated viewers.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub struct GroupDefsJoinRequestConvoView {
    ...
    pub name: String,
    pub owner: crate::api::chat::bsky::ActorDefsProfileViewBasic,
    pub viewer: GroupDefsJoinLinkViewerState,
    /// Extra fields not defined in the schema (JSON).
    #[serde(flatten)]
    pub extra: std::collections::HashMap<String, serde_json::Value>,
    /// Extra fields not defined in the schema (CBOR).
...
impl GroupDefsJoinRequestConvoView {
    ...
    pub fn encode_cbor(&self, buf: &mut Vec<u8>) -> Result<(), crate::cbor::CborError> {
        if self.extra_cbor.is_empty() {
            ...
            {
                let mut vbuf = Vec::new();
                self.viewer.encode_cbor(&mut vbuf)?;
                pairs.push(("viewer", vbuf));
            }
            {
                let mut vbuf = Vec::new();
...
/// GroupDefsJoinRequestView — A join request from the perspective of the group owner.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
