...
#[derive(Debug)]
pub struct NotificationDeclarationCborView<'a> {
    pub r#type: &'a str,
    pub allow_subscriptions: &'a str,
    pub extra_cbor: Vec<(&'a str, &'a [u8])>,
    raw_cbor: &'a [u8],
}
impl<'a> NotificationDeclarationCborView<'a> {
    #[inline]
    ...
    pub fn to_owned(&self) -> Result<NotificationDeclaration, crate::cbor::CborError> {
        Ok(NotificationDeclaration {
    ...
    pub fn decode_cbor(
        ...
        drop(entries);
        Ok(Self {
            r#type: field_type.unwrap_or("app.bsky.notification.declaration"),
            allow_subscriptions: field_allow_subscriptions.ok_or_else(|| {
                crate::cbor::CborError::InvalidCbor(
                    "missing required field 'allowSubscriptions'".into(),
                )
