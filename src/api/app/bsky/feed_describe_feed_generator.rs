...
#[derive(Debug)]
pub struct FeedDescribeFeedGeneratorFeedCborView<'a> {
    pub uri: crate::syntax::AtUriRef<'a>,
    pub extra_cbor: Vec<(&'a str, &'a [u8])>,
    raw_cbor: &'a [u8],
}
impl<'a> FeedDescribeFeedGeneratorFeedCborView<'a> {
    #[inline]
    ...
    pub fn to_owned(&self) -> Result<FeedDescribeFeedGeneratorFeed, crate::cbor::CborError> {
        Ok(FeedDescribeFeedGeneratorFeed {
...
/// A validated CBOR view. Borrowed fields cannot outlive the input.
#[derive(Debug)]
pub struct FeedDescribeFeedGeneratorLinksCborView<'a> {
    pub privacy_policy: Option<&'a str>,
    pub terms_of_service: Option<&'a str>,
    pub extra_cbor: Vec<(&'a str, &'a [u8])>,
    raw_cbor: &'a [u8],
}
impl<'a> FeedDescribeFeedGeneratorLinksCborView<'a> {
    #[inline]
    pub fn from_cbor(data: &'a [u8]) -> Result<Self, crate::cbor::CborError> {
        let mut decoder = crate::cbor::Decoder::new(data);
        let result = Self::decode_cbor(&mut decoder)?;
    ...
    pub fn to_owned(&self) -> Result<FeedDescribeFeedGeneratorLinks, crate::cbor::CborError> {
        Ok(FeedDescribeFeedGeneratorLinks {
            privacy_policy: self
                .privacy_policy
                .as_ref()
    ...
    pub fn decode_cbor(
        ...
        let start = decoder.position();
        let mut field_privacy_policy: Option<&'a str> = None;
        let mut field_terms_of_service: Option<&'a str> = None;
        let mut extra_cbor = Vec::new();
        let mut entries = decoder.map_entries()?;
        entries.try_field(
            b"\x6d\x70\x72\x69\x76\x61\x63\x79\x50\x6f\x6c\x69\x63\x79",
            |decoder| {
                field_privacy_policy = Some(decoder.text()?);
                Ok(())
            },
        )?;
        entries.try_field(
            b"\x6e\x74\x65\x72\x6d\x73\x4f\x66\x53\x65\x72\x76\x69\x63\x65",
            |decoder| {
                field_terms_of_service = Some(decoder.text()?);
                Ok(())
            },
        )?;
        while let Some(result) = entries.next_raw(|key, decoder| {
